...
If we are interested in the effet of the intervention in terms of changes in hospitaliztion rate, for example suppose the intervention in the new study is expected to reduce the patient-level hospitalization rate by at most 0.4, the potential outcome under intervention can be simulated from the following $f_1$:
$$f_1:  Y_{ij}(1) = max(Y_{ij}(0)-0.4, 0)$$
```{r rate_change, eval=FALSE}
sim_potential_outcomes = function(data, ...){
  Y0 = data[,"Y0"]
  Y1 = Y0 - 0.4 ## effect size = -0.4
  Y1[Y1<0] = 0
  re = cbind(Y0, Y1)
  colnames(re) = list("Y0", "Y1")
  return(re)
...
#### GEE when intervention effect the possion process in a ZIP model
```{r GEE2, eval=FALSE}
sim_potential_outcomes = function(data, ...){
  Y0 = data[,"Y0"]
...
CRTsearch(nrep=1e5, nt=15, nc=15, dataset=synFluData, outcome="count", clusterID="clusterID", replacement=TRUE, FUN_Ys=sim_potential_outcomes, FUN_TestStat=calc_teststat)
```
